...
In our testing case, we use one lidar sensor on the right side of the crawler. As we continously read the lidar value, we check to see if we are getting close to a wall. Using conditionals we check the crawler's distance from the wall and turn the wheel's right or left to keep the car on our center line. 
